...
*(Note: If the "Open Anyway" button doesn't appear, try to open the app and close the popup again it will be available now).*
...
- **Intelligent Pre-flight Analysis**: Prior to processing, the app reads NCCH headers to detect if a file is encrypted, decrypted, or simply missing the `NoCrypto` header flag (already decrypted but misreported).
